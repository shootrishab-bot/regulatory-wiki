...
# Same three-guard OCR safety fix as cci_scraper/extractor.py (lower DPI, hard
# page cap, timeout) -- carried over unchanged rather than re-derived, since
# the real risk it protects against is identical here: live-confirmed
# 2026-08-25 that pre-2015 Compliance Orders (e.g. ABL_2_2013.pdf) are scanned
# images with 0 chars of direct-extractable text, same as CCI's pre-2015
# corpus.
_OCR_DPI = 150
_MAX_OCR_PAGES = 15
_OCR_TIMEOUT_SECONDS = 90

# CONFIRMED live 2026-08-25 against fiuindia.gov.in/files/FAQs/faqs.html: real
# site chrome classes are .top_header (Government of India strip), .goi_text,
# .access_icon (accessibility toggles), plus a Bootstrap-style left-nav
# sidebar (.col-md-3 inside the main .row) that repeats the full section menu
# on every single page. Generic header/footer/nav tags are also stripped,
# same as cci_scraper's list, since this site uses real <nav> elements too.
BOILERPLATE_SELECTORS = [
    "header", "footer", "nav",
    ".top_header", ".goi_text", ".access_icon",
    ".navbar", ".breadcrumb", ".skip-to-content",
]
...
BOILERPLATE_TEXT_PATTERNS = [
    ...
    r"(?im)^\s*(Home|Sitemap|Site Map|Contact Us|Disclaimer|Privacy Policy|Terms of Use|Website Policies|Help)\s*$",
]
...
def extract_pdf_text(pdf_path: Path) -> Tuple[Optional[str], str]:
    ...
    (near-)nothing -- confirmed necessary for pre-2015 FIU-IND Compliance
    Orders, which are scanned images (see module docstring).
