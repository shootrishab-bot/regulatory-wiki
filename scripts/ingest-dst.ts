...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/dst_normalized.json");
...
async function main() {
  ...
  console.log(`Loaded ${all.length} total normalized DST documents. Processing ${docs.length}.\n`);

  const results = await ingestBatch(docs, "DST");
  ...
  console.log("\n" + "=".repeat(70));
  console.log(`OVERALL COUNTS (DST run, ${docs.length} of ${all.length})`);
