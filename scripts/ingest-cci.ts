...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/cci_normalized.json");
...
async function main() {
  ...
  console.log(`Loaded ${all.length} total normalized CCI documents. Processing ${docs.length}.\n`);

  const results = await ingestBatch(docs, "CCI");
  ...
  console.log("\n" + "=".repeat(70));
  console.log(`OVERALL COUNTS (CCI run, ${docs.length} of ${all.length})`);
