...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/saral_sanchar_normalized.json");

// Pass a number to test against a subset first, e.g.: npx tsx scripts/ingest-saralsanchar.ts 30
...
async function main() {
  ...
  console.log(`Loaded ${all.length} total normalized SARALSANCHAR documents. Processing ${docs.length}.\n`);

  const results = await ingestBatch(docs, "SARALSANCHAR");
  ...
  console.log("\n" + "=".repeat(70));
  console.log(`OVERALL COUNTS (SARALSANCHAR run, ${docs.length} of ${all.length})`);
