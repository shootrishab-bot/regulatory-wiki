...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/epfo_normalized.json");
...
async function main() {
  ...
  console.log(`Loaded ${all.length} total normalized EPFO documents. Processing ${docs.length}.\n`);

  const results = await ingestBatch(docs, "EPFO");
  ...
  console.log("\n" + "=".repeat(70));
  console.log(`OVERALL COUNTS (EPFO run, ${docs.length} of ${all.length})`);
