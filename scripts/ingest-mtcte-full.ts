...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/mtcte_normalized.json");

async function main() {
  const all: NormalizedDocument[] = JSON.parse(fs.readFileSync(NORMALIZED_JSON, "utf-8"));
  console.log(`Loaded ${all.length} total normalized MTCTE documents.\n`);

  const results = await ingestBatch(all, "MTCTE");
  ...
  console.log("\n" + "=".repeat(70));
  console.log("OVERALL COUNTS (full 151-document MTCTE run)");
