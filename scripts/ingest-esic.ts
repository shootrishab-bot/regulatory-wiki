...
const NORMALIZED_JSON = path.join(__dirname, "../scrapers/data/esic_normalized.json");
...
async function main() {
  ...
  console.log(`Loaded ${all.length} total normalized ESIC documents. Processing ${docs.length}.\n`);

  const results = await ingestBatch(docs, "ESIC");
  ...
  console.log("\n" + "=".repeat(70));
  console.log(`OVERALL COUNTS (ESIC run, ${docs.length} of ${all.length})`);
